...
Курсовой проект представляет собой службу доставки - бэкенд-приложение, всеми основными функциями которого можно пользоваться.
...
Все данные сохраняются в СУБД `MongoDB`.

Для реализации аутентификации пользователя используется механизм сессий и библиотека `Passport.js`.

Для онлайн-общение пользователей используется библиотека `Socket.IO`.

Запуск проекта очуществляется через `Docker`.
